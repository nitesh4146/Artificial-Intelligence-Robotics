...
class robot:
    ...
    def __repr__(self):  # allows us to print robot attributes.
        ...
        #           self.steering_noise
        #           self.distance_noise
    def move(self, motion, tolerance=0.01):  # Do not change the name of this function
        ...
        turn = distance2 * (tan(steering2)) / res.length
        ...
        # according to
        #           self.bearing_noise
    def sense(self, bearing_error = 1):  # do not change the name of this function
        Z = []

        # ENTER CODE HERE
        L = len(landmarks)
        for i in range(L):
            del_x = landmarks[i][1] - self.x
            del_y = landmarks[i][0] - self.y
            bearing = atan2(del_y, del_x) - self.orientation
...
og_robot = robot()
og_robot = generate_ground_truth(motions)
print og_robot[0]
